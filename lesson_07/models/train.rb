class Train
  ...
  def attach_wagon(wagon)
    raise "Для добавления вагона поезд должен быть остановлен" if !self.speed.zero?
    raise "Некорректный тип вагона или он уже был добавлен ранее!" if !wagon.match?(self) || self.wagons.include?(wagon)
    attach_wagon!(wagon)
  end
  ...
  def go(direction = :forward)
    raise "Маршрут не назначен" if self.route.nil?
  ...
  def validate!
    ...
    raise "Поезд с таким номером уже существует." if @@trains.has_key?(self.number.to_sym)
    true
  end
