class Train
  ...
  def attach_wagon(wagon)
    raise "Для добавления вагона поезд должен быть остановлен" unless speed.zero?
    unless wagon.match?(self) || wagons.include?(wagon)
      raise "Некорректный тип вагона или он уже был добавлен ранее!"
    end
    attach_wagon!(wagon)
  end
  ...
  def validate!
    ...
    raise "Поезд с таким номером уже существует." if @@trains.key?(number.to_sym)
    true
  end
